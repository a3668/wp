...
window.onhashchange = async function () {
  ...
  switch (tokens[0]) {
    case '#show':
      let r = await sqlFetch('blog', `SELECT id, title, body FROM posts WHERE id=${tokens[1]}`)//sqlFetch是一個在lib裡sql.js裡的函數
    ...
    case '#new':
      ...
      break
    default:
      let posts = await sqlFetch('blog', `SELECT id, title, body FROM posts`)//所有的貼文傳回來變成一個陣列
      R.list(posts)//將所有貼文呈現在畫面上
      break
  }
}
